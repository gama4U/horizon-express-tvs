...
import { deleteTravelVoucher } from "@/api/mutations/transaction.mutation";

interface Props {
	id: string;
}

export default function DeleteTravel(props: Props) {
	const { id } = props;
	const [open, setOpen] = useState(false);
	const queryClient = useQueryClient();

	const { mutate, isPending } = useMutation({
		mutationFn: async (id: string) => await deleteTravelVoucher(id),
		onSuccess() {
			toast.success("Travel voucher deleted successfully", {
				position: 'top-center',
			...
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						This action cannot be undone. This will permanently delete this travel voucher from our servers.
					</AlertDialogDescription>
				</AlertDialogHeader>
				<AlertDialogFooter>
					<AlertDialogCancel
